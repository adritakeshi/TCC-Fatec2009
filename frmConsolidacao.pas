...
begin
  Sem := Builder.SemCons; //Sem tem que receber o número do semestre
  ...
  For i:= 1 to N do
  begin
    //Criando Labels das Matérias a serem consolidadas
    ...
    //Criando CheckBoxes para as matérias aprovadas
  ...
  //Ajustando a posição dos botôes e o tamanho do form
  btnOk.Top := a + 17;
...
begin
  ...
  if Cont = 0 then
    Mensagem := 'Deseja consolidar o ' + IntToStr(sem) + 'º Semestre?'
  else
  if Cont = 1 then
    Mensagem := 'Deseja consolidar o ' + IntToStr(sem) + 'º Semestre com a matéria '
            + Mens + '?'
  else
  if Cont > 1 then
    Mensagem := 'Deseja consolidar o ' + IntToStr(sem) + 'º Semestre com as matérias '
            + Mens + '?';
  ...
  begin
    ...
    if Cont = 0 then
      ShowMessage(IntToStr(Sem) + 'º Semestre Consolidado com Sucesso.');
